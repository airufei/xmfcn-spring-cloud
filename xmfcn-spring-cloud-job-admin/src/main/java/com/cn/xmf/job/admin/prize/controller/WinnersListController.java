...
import com.cn.xmf.job.admin.prize.rpc.WinnersListService;
...
public class WinnersListController {
    ...
    @RequestMapping
    public String index() {
        return "prize/winnersList-index";
    }
    ...
    public ReturnT<String> delete(HttpServletRequest request) {
        ...
        long newId = id;
        boolean ret = winnersListService.delete(newId);
        if (ret) {
    ...
    public ReturnT<String> save(WinnersList winnersList) {
        ...
        WinnersList ret = winnersListService.save(winnersList);
        if (ret == null) {
