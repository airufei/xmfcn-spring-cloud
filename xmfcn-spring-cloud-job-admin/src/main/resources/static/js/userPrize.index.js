$(function () {
    //查询表格数据
    var userPrizeTable = $("#userPrize_table").dataTable({
        "deferRender": true,
        "processing": true,
        "serverSide": true,
        "ajax": {
            url: base_url + "/userPrize/pageList",
            type: "post",
            data: function (d) {
                var obj = {};
                obj.type = $('#type').val();
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    return function () {
                        ...
                        return '<p id="' + row.id + '" >' +
                            '<button class="btn btn-warning btn-xs job_operate" _type="userPrize_save"" type="button">' + I18n.system_opt_edit + '</button>  ' +
                            '<button class="btn btn-danger btn-xs job_operate" _type="userPrize_del" type="button">' + I18n.system_opt_del + '</button>  ' +
                            '</p>';
                    };
                }
            }
    ...
    $('#searchBtn').on('click', function () {
        userPrizeTable.fnDraw();
    });


    // 操作按钮事件
    $("#userPrize_table").on('click', '.job_operate', function () {
        var typeName;
        var url;
        var needFresh = false;
        var type = $(this).attr("_type");
        if ("userPrize_del" == type) {
            typeName = I18n.system_opt_del;
            url = base_url + "/userPrize/delete";
            needFresh = true;
        } else {
            return;
        }
   ...
   //编辑按钮事件
    $("#userPrize_table").on('click', '.job_operate', function () {
        var type = $(this).attr("_type");
        if ("userPrize_save" == type) {
            edit(this);
        }
    });

    //双击弹出编辑
    $('#userPrize_table').on('dblclick','tr',function(){
        edit(this);
    });
    ...
    var addModalValidate = $("#addModal .form").validate({
        ...
        messages: {
            ...
            imgUrl: {
                required: I18n.system_please_input + "图片"
            },
            createtime: {
        ...
        submitHandler: function (form) {
            $.post(base_url + "/userPrize/save", $("#addModal .form").serialize(), function (data, status) {
                if (data.code ==200) {
                    $('#addModal').modal('hide');
                    layer.open({
                        title: I18n.system_tips,
